...
describe("DirectorySampleTypeTransformer", () => {
  ...
  it('tags a "Melody Kits" directory with sampleType "Melody Loops" in the auto-config', async () => {
    const zipped = zipSync({
      "Loops/Melody Kits/loop.wav": strToU8("data"),
    });
    ...
    try {
      ...
      const dirRow = csv
        .split("\n")
        .find((row) => row.startsWith("Loops/Melody Kits,"));
      expect(dirRow).toBe("Loops/Melody Kits,,,Melody Loops,false");
    } finally {
      await rm(tmpDir, { recursive: true });
    }
  });
