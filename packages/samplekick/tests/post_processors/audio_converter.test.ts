...
describe("AudioConverter", () => {
  ...
  describe("WAV files", () => {
    it("runs ffmpeg with 16-bit 48kHz conversion settings", async () => {
      ...
      const converter = new AudioConverter(runFfmpeg, undefined, { targetBitDepth: BIT_DEPTH_16, targetSampleRate: SAMPLE_RATE_48000 });
    ...
    it("runs ffmpeg with 24-bit 44.1kHz conversion settings", async () => {
      const runFfmpeg = vi.fn().mockResolvedValue(undefined);
      const converter = new AudioConverter(runFfmpeg, undefined, { targetBitDepth: BIT_DEPTH_24, targetSampleRate: SAMPLE_RATE_44100 });
    ...
    it("runs ffmpeg with 32-bit 96kHz conversion settings", async () => {
      const runFfmpeg = vi.fn().mockResolvedValue(undefined);
      const converter = new AudioConverter(runFfmpeg, undefined, { targetBitDepth: BIT_DEPTH_32, targetSampleRate: SAMPLE_RATE_96000 });
  ...
  describe("onDebug", () => {
    it("calls onDebug with 'Converting <filename> to 16-bit 48 kHz WAV'", async () => {
      ...
      const converter = new AudioConverter(runFfmpeg, undefined, { onDebug, targetBitDepth: BIT_DEPTH_16, targetSampleRate: SAMPLE_RATE_48000 });
  ...
  describe("error handling", () => {
    it("cleans up the temp file when ffmpeg fails", async () => {
      ...
      const converter = new AudioConverter(runFfmpeg, vi.fn<ConvertErrorHandler>(), { targetBitDepth: BIT_DEPTH_16, targetSampleRate: SAMPLE_RATE_48000 });
    ...
    it("calls onError with the destPath and error when ffmpeg fails", async () => {
      ...
      const converter = new AudioConverter(runFfmpeg, onError, { targetBitDepth: BIT_DEPTH_16, targetSampleRate: SAMPLE_RATE_48000 });
