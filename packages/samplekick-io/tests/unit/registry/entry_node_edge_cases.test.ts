...
describe("EntryNode edge cases", () => {
  it("throws if entry path does not match node path (with parent)", () => {
    ...
    expect(
      () =>
        new EntryNode("foo/bar", createFileEntry({ path: "foo/baz" }), parent),
    ).toThrow('Entry path "foo/baz" does not match node path "foo/bar"');
  });
  ...
  it("throws if replaceEntry is called on a node with children", () => {
    ...
    expect(() => {
      parent.replaceEntry(createFileEntry({ path: "foo" }));
    }).toThrow("Cannot set entry on a node with children");
  });
  ...
  it("addNode throws if a node already exists at that path", () => {
    ...
    expect(() => parent.addNode(entry)).toThrow(
      'Node already exists at path "bar"',
    );
  });
