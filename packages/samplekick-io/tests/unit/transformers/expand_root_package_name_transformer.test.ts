...
import { createExpandRootPackageNameTransformer } from "../../../src";
import {
...
describe("createExpandRootPackageNameTransformer", () => {
  it("expands CamelCase packageName into space-separated words", () => {
    const entry = createTransformEntry({
      name: "CoolPack.zip",
      packageName: "CoolPack",
    });
  ...
  it("expands CamelCase packageName with a hyphen-separated suffix", () => {
    const entry = createTransformEntry({
      name: "CamelCase-rev1.zip",
      packageName: "CamelCase-rev1",
    });
    const transformer = createExpandRootPackageNameTransformer();
    transformer.transform(singleEntryTransformSource(entry));
  ...
  it("handles acronym runs correctly", () => {
    const entry = createTransformEntry({
      name: "MyXMLParser.zip",
      packageName: "MyXMLParser",
    });
    const transformer = createExpandRootPackageNameTransformer();
    transformer.transform(singleEntryTransformSource(entry));
  ...
  it("spaces a letter-left hyphen even when right side is a digit", () => {
    const entry = createTransformEntry({
      name: "BitPack-8bit.zip",
      packageName: "BitPack-8bit",
    });
    const transformer = createExpandRootPackageNameTransformer();
    transformer.transform(singleEntryTransformSource(entry));
  ...
  it("spaces a letter-left hyphen before a digit like CoolPack-8bit", () => {
    const entry = createTransformEntry({
      name: "CoolPack-8bit.zip",
      packageName: "CoolPack-8bit",
    });
    const transformer = createExpandRootPackageNameTransformer();
    transformer.transform(singleEntryTransformSource(entry));
