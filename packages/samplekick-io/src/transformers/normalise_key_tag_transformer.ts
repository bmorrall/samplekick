...
// Quality: major/minor/maj/min (optionally followed by [_ ]?\d+), sus2/sus4 (optionally followed by add\d+), dim (optionally followed by \d) (case-insensitive).
...
const KEY_RE =
  /(?<![a-zA-Z])(?<root>[A-G][#b]?)[_ ]?(?<quality>(?:major|minor|maj|min)(?:[_ ]?\d+)?|sus[24](?:[_ ]?add\d+)?|dim\d*)(?![a-zA-Z])/giv;
...
function keyTagReplacer(_match: string, root: string, quality: string): string {
  ...
  const normQuality = q.replace(/^major/v, "maj").replace(/^minor/v, "min");
